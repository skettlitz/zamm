...
- Capture is one command and one sentence: `backlog add 'One sentence.'` (scope defaults to `other`; the sentence is the headline). If you already know the topic, say so — `--scope domain/lobby` (or similar): the lens clusters same-subpath siblings inside their area block, headings carry the per-subpath counts, and `backlog list --scope <tag>` filters — none of which can work over an undifferentiated `other` pile. The default stays cheap and uncapped; do not invent new areas or flags for this. An idea is progressive disclosure — pipe any depth on stdin, a paragraph or a whole book: plain prose lands under `## Background` (unbounded), the lens shows only the headline with `+bg`, and `backlog show <id>` opens the record.
- One idea = one thing you would promote. Siblings you might start separately are separate records SHARING a subpath (`domain/lobby`), never one blob record and never a new taxonomy — the shared subpath is what makes them a visible cluster in the lens and one filterable family.
...
- The MARKED lane is the selected stage between latent and active: `backlog mark <id>` supersedes the idea with `marked: <date>` — it then renders in the session digest (`## Marked backlog`) and stops decaying until promoted, unmarked (`backlog unmark`, which writes the explicit `marked: no`), or tombstoned. A superseding record that omits the key INHERITS the lane; the mark date is the day first selected and survives re-ups. Decisions resolve by graph precedence: a decision on a descendant record overrides its ancestors' (never by id or timestamp — same-day ids order by random suffix), a tombstone ends the lane for everything behind it, and only genuinely parallel fork decisions fall back to the deterministic id tiebreak. Keep the lane small — it is pushed into every session, and the compiler nags past its soft cap.
