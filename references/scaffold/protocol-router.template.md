...
**Backlog (ideas; latent work):** an idea worth keeping but not starting goes into the backlog, never into a Draft plan — `backlog add 'One sentence.'` is a complete capture (pipe any depth on stdin; it lands under `## Background`), and when the topic is already known, pass `--scope domain/lobby` so siblings cluster in the lens and `backlog list --scope` can filter. One idea = one thing you would promote; siblings share a subpath, never one blob. Read `backlog list` BEFORE adding and supersede or vote instead of duplicating. Ideas decay to dormancy on their own; `backlog mark <id>` selects one for implementation (it enters the digest and stops decaying), `backlog promote <id>` turns it into a plan. The digest's one `Backlog:` line is the whole session-start exposure.
