...
1. ONE command, then ONE file read: `bash <zamm-skill>/scripts/zamm-run.sh startup`. It recompiles every tree that has changed since the last compile (usually none, and it says nothing about that) and prints two lines — what the project holds, and the path of the digest. If anything is wrong with the project it adds two more: the defect types with their counts, and the path of `zamm-memory/.compiled/zamm-defects.md`, which explains each one and names the command that fixes it. Read that file when those lines appear (reconciliation is the one that must be finished this session); it is silent, and says so, when there is nothing wrong. **Open `zamm-memory/.compiled/zamm-digest.md` with your file tool and read it whole; that is the session read, and the command output is not it.** Never `cat`/`head`/`tail` it: command output is capped by the harness and the digest would be cut silently, which reads to the session as a memory consulted rather than one truncated. Searching for something the digest does not carry: `zamm-memory/.compiled/zamm-digest-full.md` is the same ledger with no entry cap, no space budget and no decay floor — grep it, do not read it whole. Do not read the digest again this session unless records were written or merged since — your own `memory create` already recompiled. Its anatomy is in `<zamm-skill>/references/memory-reading.md`; the rules that must not wait:
